...
router.get('/', ensureAuthenticated, async function(req, res, next) {
  ...
  if (req.query.date) {
    ...
    matchObj.date = {
      $gte: new Date(moment(req.query.date).subtract(24, 'hours').format()),
      $lte: new Date(moment(req.query.date).add(1, 'minutes').format())
    }
  ...
  if (workouts !== undefined && workouts.length !== 0) {
    ...
    for (let i = 0; i < workouts.length; i++) {
      ...
      if (!currentWorkoutDate.isBefore(moment().subtract(24, 'hours'))) {
        console.log('current workout date: ' + currentWorkoutDate.format("YYYY-MM-DDTHH:mm"));
        console.log('24 hours ago: ' + moment().subtract(24, 'hours').format("YYYY-MM-DDTHH:mm"));
        continue;
      }
...
router.delete('/type/:type', ensureAuthenticated, async function (req, res, next) {
  console.log('user ' + req.session.passport.user + 
    ' deleting all workouts and exercises for type: ' + req.params.type);
